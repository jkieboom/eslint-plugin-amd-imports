...
function findConfigBase(directory) {
  let visited = [];

  while (true) {
    if (eslintrcCache[directory]) {
      return eslintrcCache[directory];
    }

    visited.push(directory);

    if (fs.existsSync(path.join(directory, ".eslintrc.json"))) {
      for (const seen of visited) {
        eslintrcCache[seen] = directory;
      }
...
exports.getBasePath = function getBasePath(context, node) {
  let configBase = findConfigBase(path.dirname(context.getFilename(node)));
